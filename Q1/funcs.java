...
public class funcs {
    List<Contat> AgendaArrayList = new ArrayList<Contat>();    
    ...
    // Não tem como colocar hashmap com chaves duplicadas
    // Então tem que instanciar o objeto com as chaves dos endereços, telefones e emails (é a solução que eu pensei que funcionaria)
    // Você passa o get do index do elemento(email, tel, end etc) no ToString  que ele imprime o hashmap(não sei se apenas colocar a váriavel ele imprime direto)
    // End tem seu próprio toString dai -- fica tipo contat.toString(end.toString())
    ...
    public void ListaContato(){
        ...
        for (Contat t: AgendaArrayList) {
            System.out.println( "Código do contato: "+ AgendaArrayList.get(t.getIndexContat())+ "\n");
